...
# classe définie finalement dans la classe fille
...
# classes filles on ne définie ici que la méthode de evolnotejoueurs
class Attaquant(Joueur) :
    ...
    #on évolue la note des joeurs en fonction de si ce sont des attaquants on augmente leur note si ils ont marqué plein de buts
    def evol_notejoueur(self,resultat):
...
class Defenseur(Joueur):
    ...
    # on évolue la note des joeurs en fonction de si ce sont des defenseurs on augmente leur notes si ils ont par pris beaucoup de buts
    def evol_notejoueur(self,resultat):
